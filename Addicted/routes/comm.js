...
router.post("/addcomm", async (req, res, next) => {
    try {
        ...
        const { id_comm, description, id_user, id_post } = req.body;
        
      const po = await post.findOne({id_post});
    ...
    try {
        ...
        const post=req.body.name;
          const mailOptions = {
            ...
            subject: 'Notice mail',
            html: 
                '<p>Hello, We would like to give you a notice that your comment doesn"t meat with our comunity gide line so we deleted it</p>'
          };
